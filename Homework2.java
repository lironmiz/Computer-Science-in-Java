	...
	public static void main(String[] args) 
	{
		...
		do 
		{
			...
			case 1: 
			{
				...
				int idCopy = id;
				int numLength;
				for(numLength = 0; idCopy > 0 && numLength < 10; numLength++)
				{
					idCopy /= 10;
				}
				if (numLength != 8 && numLength != 9)
				{
				...
				boolean isMultipleByOne = (numLength != 9);
				int totalSum = 0;
				
				for (int i = 0; i < numLength - 1; i++)
				{
					int rightDigit = id % 10;
					id /= 10;
					if(!isMultipleByOne)
					{
						rightDigit *= 2;
						if(rightDigit > 9)
						{
							rightDigit = rightDigit % 10 + rightDigit / 10;
						}						
					}
					
					totalSum += rightDigit;
					isMultipleByOne = !isMultipleByOne;
				}
				
				int resultDigit = totalSum % 10 == 0 ? 0 : 10 - (totalSum % 10);
				// print to user if the id is fine or not 
